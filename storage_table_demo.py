...
resourcegroup_name = 'pls'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'pls'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
car.price = 18000
table_service.insert_entity('itemstable', car)
print('Created entry for Subaru...\n')
...
coffee = Entity()
coffee.PartitionKey = 'coffeeinv'
coffee.RowKey = '005'
coffee.brand = 'Pikes Peak'
coffee.flavor = 'Medium Roast'
coffee.size = '8 oz'
coffee.price = 2.50
table_service.insert_entity('itemstable', coffee)
print('Created entry for Pikes Peak...\n')
time.sleep(1)

coffee = Entity()
coffee.PartitionKey = 'coffeeinv'
coffee.RowKey = '006'
coffee.brand = 'Columbia'
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'carlist'", select='make,model,year,color,price')
for item in items:
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeeinv'", select='brand,flavor,size,price')
for item in items:
